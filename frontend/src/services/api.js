...
export const landsAPI = {
  ...
  updateProject: async (landId, projectData) => {
    const response = await api.put(`/lands/${landId}`, projectData);
    return response.data;
  },
  ...
  updateLandVisibility: async (landId, visibility) => {
    const response = await api.put(`/lands/${landId}/visibility`, {
      visibility,
    });
    return response.data;
  },
  ...
  getProjectDetailsWithTasks: async (projectId) => {
    const response = await api.get(
      `/lands/admin/projects/${projectId}/details`
    );
    return response.data;
  },
};
...
export const reviewsAPI = {
  saveReviewStatus: async (landId, reviewerRole, reviewData) => {
    const response = await api.post(
      `/reviews/land/${landId}/role/${reviewerRole}`,
      reviewData
    );
    return response.data;
  },
...
export const documentsAPI = {
  ...
  uploadSubtaskDocument: async (subtaskId, formData) => {
    const response = await api.post(
      `/documents/subtask/${subtaskId}/upload`,
      formData,
      {
        headers: {
          "Content-Type": "multipart/form-data",
...
export const investorsAPI = {
  ...
  getLandInterests: async (landId, params = {}) => {
    const response = await api.get(`/investors/land/${landId}/interests`, {
      params,
    });
    return response.data;
  },
...
export const taskAPI = {
  ...
  getTaskStatusByProject: async (landId) => {
    ...
    try {
      ...
      console.log(
        "🔧 API: Project review endpoint failed, trying fallback:",
        error
      );
            ...
            response.data.map(async (task) => {
              try {
                ...
                const subtasksResponse = await api.get(
                  `/tasks/${task.task_id}/subtasks`
                );
                console.log(
                  `🔧 API: Subtasks for task ${task.task_id}:`,
                ...
                return {
                  ...
                  assigned_role:
                    task.assigned_role || task.role || task.reviewer_role,
                };
              } catch (err) {
                console.error(
                  `🔧 API: Error fetching subtasks for task ${task.task_id}:`,
                  err
                );
