...
class WebSocketService {
   ...
   */
  async connect(token, baseUrl = null) {
    if (this.isConnected) {
    ...
    if (!resolvedBase) {
      const apiBase = (import.meta && import.meta.env && import.meta.env.VITE_API_BASE_URL) ? import.meta.env.VITE_API_BASE_URL : '';
      if (apiBase) {
        resolvedBase = apiBase.replace('http://', 'ws://').replace('https://', 'wss://').replace(/\/api$/, '');
      } else {
        resolvedBase = window.location.origin.replace('http://', 'ws://').replace('https://', 'wss://');
      }
    }
    const wsUrl = `${resolvedBase}/ws?token=${encodeURIComponent(token)}`;
